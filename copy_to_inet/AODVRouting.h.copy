...
class INET_API AODVRouting: public cSimpleModule,
        public ILifecycle,
...
protected:
    ...
    public:
        ...
        unsigned int rreqID;
        RREQIdentifier(const IPv4Address& originatorAddr, unsigned int rreqID) :
    ...
    //Statistic
    int SentAODVPacktes;
    int ReceivedAODVPackets;
    static simsignal_t sentRREQSignal;
    static simsignal_t sentRREPSignal;
    static simsignal_t sentRERRSignal;

    static simsignal_t receivedRREQSignal;
    static simsignal_t receivedRREPSignal;
    static simsignal_t receivedRERRSignal;
    cStdDev numReceivedAODVPackets;
    cStdDev numSentAODVPackets;
    double numberOfLinkBreaks;
    int sentRERR;
...
protected:
    ...
    AODVRREP *createRREP(AODVRREQ *rreq, IPv4Route *destRoute,
            IPv4Route *originatorRoute, const IPv4Address& sourceAddr);
    ...
    void handleRREQ(AODVRREQ *rreq, const IPv4Address& sourceAddr,
    ...
    void sendRREQ(AODVRREQ *rreq, const IPv4Address& destAddr,
            unsigned int timeToLive);
    void sendRREPACK(AODVRREPACK *rrepACK, const IPv4Address& destAddr);
    void sendRREP(AODVRREP *rrep, const IPv4Address& destAddr,
            unsigned int timeToLive);
    void sendGRREP(AODVRREP *grrep, const IPv4Address& destAddr,
            unsigned int timeToLive);

    /* Control Packet forwarders */
    void forwardRREP(AODVRREP *rrep, const IPv4Address& destAddr,
            unsigned int timeToLive);
    ...
    virtual Result datagramPreRoutingHook(IPv4Datagram *datagram,
    ...
    virtual Result datagramForwardHook(IPv4Datagram *datagram,
            const InterfaceEntry *inputInterfaceEntry,
            const InterfaceEntry *& outputInterfaceEntry,
            IPv4Address& nextHopAddress);
    virtual Result datagramPostRoutingHook(IPv4Datagram *datagram,
            const InterfaceEntry *inputInterfaceEntry,
            const InterfaceEntry *& outputInterfaceEntry,
            IPv4Address& nextHopAddress) {
        return ACCEPT;
    }
    virtual Result datagramLocalInHook(IPv4Datagram *datagram,
            const InterfaceEntry *inputInterfaceEntry) {
        return ACCEPT;
    }
    virtual Result datagramLocalOutHook(IPv4Datagram *datagram,
            const InterfaceEntry *& outputInterfaceEntry,
            IPv4Address& nextHopAddress) {
        Enter_Method
    ...
    void sendAODVPacket(AODVControlPacket *packet, const IPv4Address& destAddr,
            unsigned int timeToLive, double delay);
    void clearState();
